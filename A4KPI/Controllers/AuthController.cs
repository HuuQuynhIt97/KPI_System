...
namespace A4KPI.Controllers
{
    public class AuthController : ApiControllerBase
    {
        ...
        {
            return Ok(await _authService.CheckPassword(username, password));
        }

        [AllowAnonymous]
        [HttpPost]
        public async Task<IActionResult> LoginAnonymous(UserForLoginDto userForLoginDto)
        {
            var userFromRepo = await _authService.LoginAnonymous(userForLoginDto.Username);
            if (userFromRepo == null)
                return Unauthorized();
            var claims = new[]
            {
