...
import { Column, ColumnModel, GridComponent, IEditCell, QueryCellInfoEventArgs } from '@syncfusion/ej2-angular-grids';
...
export class PdcaStringTypeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription.push(this.todolist2Service.currentUploadMessage.subscribe(message =>
    {
      if (message) {
  ...
  post(submitted) {
    ...
    const actions = this.dataAdd.map(x => {
      return {
        ...
        target: x.target,
        content: x.content,
        deadline: this.datePipe.transform(x.deadline, 'MM/dd/yyyy HH:mm:ss'),
  ...
  save(submitted) {
    ...
    const request = {
      ...
      result: this.result,
      userId: this.userId,
      currentTime: this.datePipe.transform(this.currentTime, 'MM/dd/yyyy HH:mm:ss'),
    }
