...
import { Column, GridComponent, IEditCell } from '@syncfusion/ej2-angular-grids';
...
export class PlanComponent implements OnInit, AfterViewInit {
  ...
  submits(){
    ...
    const actions = this.dataAdd.map(x => {
      return {
        ...
        target: x.target,
        content: x.content,
        deadline: typeof(x.deadline) != "string" ? (x.deadline as Date).toLocaleDateString('en-US'): x.deadline,
        accountId: +JSON.parse(localStorage.getItem('user')).id,
        kPIId: this.data.id,
        statusId: x.statusId,
        createdTime: x.createdTime,
        modifiedTime: this.datePipe.transform(new Date(), 'yyyy/MM/dd HH:mm:ss')
      }
    })
    const request = {
      actions: actions,
    ...
    forkJoin([post, submitKPINew]).subscribe(response => {
      ...
      const checker = arr.every(Boolean);
      if (checker) {
  ...
  loadData() {
    ...
    this.todolist2Service.getActionsForL0(this.data.id || 0, this.userId).subscribe(res => {
      ...
      this.targetValue = this.target?.value === undefined ? "" : this.target?.value;
      this.targetYTDValue = this.targetYTD?.value === undefined ? "" : this.targetYTD?.value ;
    });
  }
