import { PdcaStringTypeMeetingComponent } from './pdcaStringTypeMeeting/pdcaStringTypeMeeting.component';
import { PdcaMeetingComponent } from './pdcaMeeting/pdcaMeeting.component';
...
@Component({
  selector: 'app-meeting-revise',
  templateUrl: './meeting-revise.component.html',
  styleUrls: ['./meeting-revise.component.scss'],
  providers: [DatePipe]
})
export class MeetingReviseComponent implements OnInit,OnDestroy {
  ...
  loadDataL0() {
    this.gridData = [];
    this.todolist2Service.l0Revise(this.currentTimeRequest, this.userId).subscribe(data => {
  ...
  openUpdatePdcaModalComponent(data, text) {
    if(data.typeText !== 'string') {
      const modalRef = this.modalService.open(PdcaMeetingComponent, { size: 'xxl', backdrop: 'static', keyboard: false });
      modalRef.componentInstance.data = data;
      modalRef.componentInstance.currentTime = this.currentTime;
      ...
      const modalRef = this.modalService.open(PdcaStringTypeMeetingComponent, { size: 'xxl', backdrop: 'static', keyboard: false });
      modalRef.componentInstance.data = data;
      modalRef.componentInstance.currentTime = this.currentTime;
      modalRef.componentInstance.text = text;
      modalRef.result.then((result) => {
